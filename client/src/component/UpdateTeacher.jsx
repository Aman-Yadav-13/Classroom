...
const UpdateTeacher = () => {
    // State variables for holding input values and loading status
    ...
    const handleClick = async () => {
        ...
        try {
            // Send PUT request to update teacher
            const response = await axios.put('https://classroom-ozmt.onrender.com/api/update-teacher', {
                id: email,
                name: name,
                password: password,
            ...
            // Show success message and navigate to the principal page
            alert('Teacher updated successfully...');
            navigate(`/principal/${id}`);
        } catch (err) {
            // Log error and show failure message
            console.log('Teacher not updated : ', err);
        ...
        <div className="flex justify-center items-center h-screen">
            <div className="artboard h-[70%] w-[20%] bg-base-300 flex items-center flex-wrap justify-center m-[5%] rounded-lg">
                ...
                <div>
                    ...
                    <button className="btn hover:bg-primary text-white" onClick={handleClick}>Update Teacher</button>
                </div>
            </div>
        </div>}
        </div>
